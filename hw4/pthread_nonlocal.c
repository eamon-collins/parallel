...
int main(int argc, char **argv) {
	...
	for (i = 0; i < nrows+2; i++){
		cells[0][i] = (float *) malloc((ncols+2) * sizeof(float));
		cells[1][i] = (float *) malloc((ncols+2) * sizeof(float));
	}
...
/*	if(id == 0) {
		cells[0][0] = (float *)malloc((num_cols+2) * sizeof(float));
		cells[1][0] = (float *)malloc((num_cols+2) * sizeof(float));
	}
	if (id == thread_count - 1){
		cells[0][num_rows + 1] = (float *)malloc((num_cols+2) * sizeof(float));
		cells[1][num_rows + 1] = (float *)malloc((num_cols+2) * sizeof(float));
	}
	for(i = rowStart; i < rowEnd; i++){
		cells[0][i] = (float *)malloc((num_cols+2) * sizeof(float));
		cells[1][i] = (float *)malloc((num_cols+2)* sizeof(float));
	}**/
...
//		}
	...
	if (id == thread_count - 1){
		free(cells[0][num_rows + 1]);
		free(cells[1][num_rows + 1]);
	}
	for(i = rowStart; i < rowEnd; i++){
		free(cells[0][i]);
		free(cells[1][i]);
