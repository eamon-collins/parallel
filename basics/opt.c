...
int main(int argc, char *argv[])
{
    ...
    cut = cut * cut;
    //take the inverse of a once to remove all the needless divisions inside the loop body
    double inverse_a = 1.0 / a;
    double qi, coords0, coords1, coords2;
    //set loop bounds to 0 and <natom and took away the extraneous -1 everytime they were used
    //also took away to if(i < j) branch and simply set the loop bound at j<i
    for (i=0; i<natom; ++i)
    {
	//taking coords lookups dependent on i out of the inner loop so it only has to be done once per i loop
	coords0 = coords[0][i];
	coords1 = coords[1][i];
	coords2 = coords[2][i];
	qi = q[i];
